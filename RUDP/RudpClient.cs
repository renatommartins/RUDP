...
namespace RUDP
{
	...
	public class RudpClient
	{
		...
		public RudpClient(ushort appId, ISocket socket = null) : this(appId)
		{
			_socket = socket;
		}
		...
		public List<(ushort seqNum, PacketResult rudpEvent)> GetPacketResults()
		{
			...
			resultList.Sort(delegate((ushort seqNum, PacketResult rudpEvent) result1,
				(ushort seqNum, PacketResult rudpEvent) result2)
			{
			...
			if (Packet.SequenceNumberGreaterThan((ushort) (_nextSeqNumber - 1), (ushort) (_lastAckSeqNum + 32)))
			{
				...
				lock (_sendDataQueue)
					...
					if (_sendDataQueue.Count > 0)
					{
						...
						{
							// Concatenates all pending data in a single byte array.
		...
		internal void ReceiveUpdate(Packet packet)
		{
			if (packet.Validate(AppId, _lastRemoteSeqNumber))
			{
				...
				_remotePacketAcks.RemoveWhere(seqNum =>
					!Packet.SequenceNumberGreaterThan(seqNum, (ushort) (packet.SequenceNumber - 33)));
				...
				lock (_pendingAckPackets)
				{
					...
					if (_pendingAckPackets.ContainsKey(packet.AckSequenceNumber))
					{
						...
						_pendingAckPackets[packet.AckSequenceNumber]
							?.Invoke(this, packet.AckSequenceNumber, PacketResult.Successful);
						...
						// RTT measurement.
						_rttList.Add(packet.AckSequenceNumber,
							(int) (_rttStopwatch.ElapsedMilliseconds - _pendingPacketsTime[packet.AckSequenceNumber]));
					...
					for (ushort i = (ushort) (packet.AckSequenceNumber - 1), j = 0;
					     Packet.SequenceNumberGreaterThan(i, (ushort) (packet.AckSequenceNumber - 33));
					     i--, j++)
					{
					...
					foreach (var pair in _pendingAckPackets.Where(seqNum =>
						         !Packet.SequenceNumberGreaterThan(seqNum.Key,
							         (ushort) (packet.AckSequenceNumber - 33))).ToList())
					{
						// Calls the registered callback for this packet's result.
				...
				foreach (var pair in _rttList.Where(seqNum =>
						         !Packet.SequenceNumberGreaterThan(seqNum.Key,
							         (ushort) (packet.AckSequenceNumber - 33)))
					         .ToList())
